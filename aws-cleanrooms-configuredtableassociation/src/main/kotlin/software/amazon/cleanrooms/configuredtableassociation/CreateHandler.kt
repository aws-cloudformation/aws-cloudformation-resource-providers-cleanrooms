...
class CreateHandler : BaseHandler<CallbackContext?>() {
    ...
    override fun handleRequest(
        ...
        logger.log("Inside CREATE handler for ConfiguredTableAssociation resource. Request AWS AccountId:$requestAccountId")
    ...
    private fun processCreateRequest(
        ...
        return try {
            with(resourceModel) {
                ...
                val cta = createConfiguredTableAssociation(this, combinedTags, proxy, cleanRoomsClient)
                logger.log("ConfiguredTableAssociation Arn from API: ${cta.arn()}")

                // Updating the readOnly properties for the resourceModel.
                resourceModel.apply {
                    configuredTableAssociationIdentifier = cta.id()
                    arn = cta.arn()
                }
