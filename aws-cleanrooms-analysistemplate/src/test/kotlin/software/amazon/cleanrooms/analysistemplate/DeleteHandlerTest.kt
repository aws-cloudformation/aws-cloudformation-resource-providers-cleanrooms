...
class DeleteHandlerTest {
    ...
    fun `DeleteHandler returns IN PROGRESS after deleting membership`() {
        ...
        every {
            mockProxy.injectCredentialsAndInvokeV2(
                ofType(DeleteAnalysisTemplateRequest::class),
                any<Function<DeleteAnalysisTemplateRequest, DeleteAnalysisTemplateResponse>>()
            )
        } returns DeleteAnalysisTemplateResponse.builder().build()
    ...
    fun `DeleteHandler returns IN_PROGRESS if exception thrown from ListHandler during stabilization`() {
        ...
        } returns GetAnalysisTemplateResponse.builder().analysisTemplate(TEST_ANALYSIS_TEMPLATE_REQUIRED_FIELDS).build()
