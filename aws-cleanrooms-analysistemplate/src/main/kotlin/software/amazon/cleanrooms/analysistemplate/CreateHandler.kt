...
class CreateHandler : BaseHandler<CallbackContext?>() {
    ...
    override fun handleRequest(
        ...
        logger.log("Inside CREATE handler for AnalysisTemplate resource. Request AWS AccountId:$requestAccountId")
    ...
    private fun processCreateRequest(
        ...
        return try {
            with(resourceModel) {
                ...
                val at = createAnalysisTemplate(this, combinedTags, proxy, cleanRoomsClient)
                logger.log("AnalysisTemplate Arn from API: ${at.arn()}")

                // Updating the readOnly properties for the resourceModel.
                resourceModel.apply {
                    analysisTemplateIdentifier = at.id()
                    arn = at.arn()
                }
