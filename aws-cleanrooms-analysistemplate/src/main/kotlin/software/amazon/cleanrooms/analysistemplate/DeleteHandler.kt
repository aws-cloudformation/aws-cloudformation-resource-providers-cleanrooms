...
import software.amazon.cleanrooms.analysistemplate.HandlerCommon.CALLBACK_DELAY_IN_SECONDS
import software.amazon.cleanrooms.analysistemplate.HandlerCommon.verifyListResourceFound
...
class DeleteHandler : BaseHandler<CallbackContext?>() {
    override fun handleRequest(
        ...
        logger.log("Inside DELETE handler for AnalysisTemplate resource. Request AWS AccountId:$requestAccountId")
    ...
    private fun processDeleteRequest(
        ...
        return try {
            with(resourceModel) {
                logger.log("DeleteHandler from resourceModel: $analysisTemplateIdentifier")
    ...
    private fun waitForDeleteStabilization(
        ...
        if (currentCallbackContext.stabilizationRetriesRemaining < 0) {
            val notStabilizedException = CfnNotStabilizedException(ResourceModel.TYPE_NAME, desiredResourceModel.analysisTemplateIdentifier)
