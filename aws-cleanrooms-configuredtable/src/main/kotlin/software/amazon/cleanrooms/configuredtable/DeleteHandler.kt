...
import software.amazon.cleanrooms.configuredtable.HandlerCommon.CALLBACK_DELAY_IN_SECONDS
import software.amazon.cleanrooms.configuredtable.HandlerCommon.verifyListResourceFound
...
class DeleteHandler : BaseHandler<CallbackContext?>() {
    ...
    override fun handleRequest(
        ...
        logger.log("Inside DELETE handler for ConfiguredTable resource. Request AWS AccountId:$requestAccountId")
    ...
    private fun processDeleteRequest(
        ...
        return try {
            with(resourceModel) {
                logger.log("DeleteHandler ConfiguredTableIdentifier from resourceModel: $configuredTableIdentifier")
                deleteConfiguredTable(configuredTableIdentifier, proxy, cleanRoomsClient)

                val currentCallbackContext = CallbackContext(stabilizationRetriesRemaining = HandlerCommon.NUMBER_OF_STATE_POLL_RETRIES,
